...
export function LeagueJoinPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true)
    try {
      await dataClient.joinLeagueByCode({ code, userId: user!.id })
  ...
  return (
    <div className="mx-auto max-w-sm rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
      <h1 className="mb-4 text-xl font-semibold text-slate-900">Join a league</h1>
      <form onSubmit={handleSubmit} className="space-y-3">
        <div>
          <label htmlFor="invite-code" className="mb-1 block text-sm font-medium text-slate-700">
            Invite code
          </label>
          <input
            id="invite-code"
            ...
            className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm font-mono uppercase tracking-widest focus:border-slate-500 focus:outline-none"
          />
        </div>
